...
(function ($) { 
	$(document).ready(function(){   
	    $('#toggle-hidden-filters').click(function(){
	        $('.hide-filters').toggle('fast');
	    });
	    $('#edit-submit-taxonomy-term').hide();
	    $('#views-exposed-form-taxonomy-term-page select').change(function(){
	        $('#views-exposed-form-taxonomy-term-page').submit();
	    });
	    var width = $("body").width(), columnWidth = 315, gutter = 5;
	    if(width > 1024){
	    
	    }else if(width <= 1024 && width > 768){
	        columnWidth = 315, gutter = 5;
	    }else if(width <= 768 && width > 570){
	        columnWidth = 350, gutter = 2;
	    }
        ...
        $('.view-search-view .view-content').masonry({
			itemSelector: '.views-row',
			columnWidth: 315,
			gutter: 5
		}).fadeIn();
		if($(".banner-description-title p").length > 1 && width > 570){
		    $(".banner-description-title").children().hide();
		    $(".banner-description-title p:first").show();
		    $("#banner .read-more").show();
		    var i = false;
		    $("#banner .read-more").click(function(){
		        if(i == false){
		            $(".banner-description-title").children().show();
		            $("#banner .read-more a").text("Read Less");
		            i = true;
		        }else{
		            $(".banner-description-title").children().hide();
		            $(".banner-description-title p:first").show();
		            $("#banner .read-more a").text("Read More");
		            i = false;
		        }
		    });
		}
